...
USER_AGENT = "decoder-pi-sparql/1.0 (+https://github.com/earthcube)"
DEFAULT_TIMEOUT = 30
...
def execute_sparql(endpoint: str, query: str, timeout: int = DEFAULT_TIMEOUT) -> dict:
    ...
    try:
        ...
        print(
            ...
            "From the decoder-pi repo root run: uv sync",
            file=sys.stderr,
        )
        sys.exit(1)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Query a SPARQL endpoint with curated schema.org templates "
            "or ad-hoc SPARQL."
        )
    )
