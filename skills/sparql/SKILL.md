...
name: sparql
description: >
  Query schema.org RDF graphs via SPARQL. Use when the user wants to run SPARQL,
  inspect a triplestore, list datasets, inventory variableMeasured or depth
  (DepBelowSurf), explore named graphs, or build schema.org SPARQL against an
  endpoint URL. Triggers: SPARQL, Oxigraph, QLever, /query, /skill:sparql,
...
license: Apache-2.0
compatibility: python3, uv, SPARQLWrapper (decoder-pi pyproject.toml)
metadata:
  project: decoder-pi
  version: "1.0"
  author: DOOS / GoFAIR US (adapted)
  source: doos-sparql
---
...
Adapted from the DOOS `doos-sparql` skill. Templates still target schema.org +
GeoSPARQL patterns used in DOOS/Oxigraph named graphs.
...
Run from the **decoder-pi repo root** (`uv` resolves `SPARQLWrapper` from this
repo's `pyproject.toml`). Paths below are relative to that root:
...
uv run python skills/sparql/scripts/sparql_query.py query \
  --endpoint http://localhost:7878/query \
...
# Any .rq file
uv run python skills/sparql/scripts/sparql_query.py file \
  --endpoint http://localhost:7878/query \
  --query-file skills/sparql/queries/probe_triples.rq
```
...
**Deps:** `SPARQLWrapper` via this repo's `pyproject.toml`. First run:
...
Metadata in this domain is primarily **schema.org** + **GeoSPARQL**, often in
**named graphs**.
...
Most Oxigraph loads put triples in **named graphs**. Prefer:
...
| Missing SPARQLWrapper | From repo root: `uv sync` |
...
- Confuse this skill with SHACL validation
